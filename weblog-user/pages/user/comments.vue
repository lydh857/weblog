<template>
<div class="comments-page" :class="{ 'page-entered': pageEntered }">
    <div class="page-header">
      <NuxtLink to="/user" class="back-link">
        <Icon name="heroicons:arrow-left-20-solid" size="18" /> 个人中心
      </NuxtLink>
      <div class="header-row">
        <div class="header-left">
          <h1 class="page-title">
            <Icon name="heroicons:chat-bubble-left-ellipsis-20-solid" size="20" />
            我的评论
          </h1>
          <span class="page-total">共 {{ total }} 条</span>
          <template v-if="managing">
            <button class="toolbar-btn" @click="toggleSelectAll">{{ isAllSelected ? '取消全选' : '全选当前页' }}</button>
            <Transition name="fade">
              <button v-if="selectedIds.size > 0" class="toolbar-btn danger-btn" @click="showBatchConfirm = true">
                <Icon name="heroicons:trash-16-solid" size="14" />
                删除 ({{ selectedIds.size }})
              </button>
            </Transition>
          </template>
        </div>
        <button class="manage-btn" @click="managing = !managing">
          <template v-if="managing"><Icon name="heroicons:x-mark-16-solid" size="16" /> 取消</template>
          <template v-else><Icon name="heroicons:cog-6-tooth-16-solid" size="16" /> 管理</template>
        </button>
      </div>
    </div>

    <UnifiedPageLoader v-if="loading" plain :text="''" />

    <template v-else-if="comments.length">
      <div
        v-for="(comment, index) in comments"
        :key="comment.id"
        class="comment-card"
        :class="{
          selected: selectedIds.has(comment.id),
          managing,
          'card-entered': listEntered,
          'comment-pending': comment.status === 'pending',
          'comment-rejected': comment.status === 'rejected',
        }"
        :style="{ '--enter-index': index }"
        @click="managing && toggleSelect(comment)"
      >
        <div class="row-top">
          <NuxtLink v-if="comment.postSlug" :to="`/post/${comment.postSlug}#comment-${comment.id}`" class="post-link" @click.stop>
            {{ comment.postTitle || '未知文章' }}
          </NuxtLink>
          <span v-else class="post-link disabled">{{ comment.postTitle || '文章已删除' }}</span>
          <span class="meta-likes"><Icon name="heroicons:heart-16-solid" size="12" /> {{ comment.likeCount }}</span>
          <span v-if="comment.status === 'pending'" class="status-badge status-badge--pending">待审核</span>
          <span v-else-if="comment.status === 'rejected'" class="status-badge status-badge--rejected">已拒绝</span>
          <time class="meta-time">{{ formatRelativeTime(comment.createTime) }}</time>
        </div>

        <div class="comment-text">
          <span v-if="comment.parentId && comment.parentId > 0 && comment.replyToNickname" class="reply-prefix">回复 @{{ comment.replyToNickname }}：</span>{{ comment.content }}
        </div>

        <div v-if="comment.status === 'rejected' && comment.rejectReason" class="reject-reason">
          <Icon name="heroicons:exclamation-circle-16-solid" size="14" />
          拒绝原因：{{ comment.rejectReason }}
        </div>

        <button v-if="!managing && isCommentDeletable(comment)" class="delete-btn" title="删除" @click.stop="pendingDeleteId = comment.id; showDeleteConfirm = true">
          <Icon name="heroicons:trash-16-solid" size="14" />
        </button>
      </div>

      <Pagination :total="total" :current-page="currentPage" :page-size="pageSize" :page-size-options="[10,20,30]" @update:current-page="handlePageChange" @update:page-size="handleSizeChange" />
    </template>

    <div v-else class="empty-state">
      <Icon name="heroicons:chat-bubble-left-ellipsis-20-solid" size="48" />
      <p>还没有发表过评论</p>
    </div>

    <ConfirmDialog :visible="showDeleteConfirm" message="确定删除这条评论吗？删除后不可恢复。" @update:visible="showDeleteConfirm = $event" @confirm="confirmDelete" />
    <ConfirmDialog :visible="showBatchConfirm" :message="`确定删除选中的 ${selectedIds.size} 条评论吗？`" @update:visible="showBatchConfirm = $event" @confirm="confirmBatchDelete" />
  </div>
</template>

<script setup lang="ts">
import { commentApi, type CommentVO } from '~/api/content/comment'
import { useUserStore } from '~/stores/user'
import { useLoginModal } from '~/composables/modal/useLoginModal'
import { useMessage } from '~/composables/modal/useMessage'
import { formatRelativeTime } from '~/utils/content/format'
import { scrollToTopOnMobilePagination } from '~/utils/navigation/paginationScroll'
import Pagination from '~/components/common/Pagination.vue'
import ConfirmDialog from '~/components/common/ConfirmDialog.vue'

useHead({ title: '我的评论' })

const userStore = useUserStore()
const comments = ref<CommentVO[]>([])
const loading = ref(true)
const currentPage = ref(1)
const pageSize = ref(10)
const total = ref(0)
const selectedIds = ref<Set<number>>(new Set())
const managing = ref(false)
const pageEntered = ref(false)
const listEntered = ref(false)

const showDeleteConfirm = ref(false)
const showBatchConfirm = ref(false)
const pendingDeleteId = ref(0)

const deletableComments = computed(() => comments.value.filter(isCommentDeletable))
const isAllSelected = computed(() => deletableComments.value.length > 0 && deletableComments.value.every(c => selectedIds.value.has(c.id)))

watch(managing, (val) => { if (!val) selectedIds.value = new Set() })

function isCommentDeletable(comment: CommentVO) {
  return comment.status !== 'pending'
}

function toggleSelect(comment: CommentVO) {
  if (!isCommentDeletable(comment)) return
  const s = new Set(selectedIds.value)
  if (s.has(comment.id)) s.delete(comment.id)
  else s.add(comment.id)
  selectedIds.value = s
}
function toggleSelectAll() {
  if (isAllSelected.value) selectedIds.value = new Set()
  else selectedIds.value = new Set(deletableComments.value.map(c => c.id))
}

const message = useMessage()

async function confirmDelete() {
  if (!pendingDeleteId.value) return
  const target = comments.value.find(comment => comment.id === pendingDeleteId.value)
  if (target && !isCommentDeletable(target)) {
    message.warning('待审核评论不能删除，请等待审核完成')
    pendingDeleteId.value = 0
    return
  }
  try {
    await commentApi.delete(pendingDeleteId.value)
    selectedIds.value.delete(pendingDeleteId.value)
    await loadData(currentPage.value)
  } catch { message.error('删除失败，请稍后重试') }
  pendingDeleteId.value = 0
}

async function confirmBatchDelete() {
  const ids = Array.from(selectedIds.value)
  if (!ids.length) return
  const hasPending = comments.value.some(comment => ids.includes(comment.id) && !isCommentDeletable(comment))
  if (hasPending) {
    message.warning('已选中待审核评论，无法执行删除')
    return
  }
  try {
    await commentApi.batchDelete(ids)
    selectedIds.value = new Set()
    await loadData(currentPage.value)
  } catch { message.error('删除失败，请稍后重试') }
}

function handlePageChange(page: number) {
  selectedIds.value = new Set()
  loadData(page)
  scrollToTopOnMobilePagination()
}

function handleSizeChange(size: number) {
  pageSize.value = size
  selectedIds.value = new Set()
  loadData(1)
  scrollToTopOnMobilePagination()
}

async function loadData(page = 1) {
  if (!userStore.isLoggedIn) { useLoginModal().open(); navigateTo('/'); return }
  loading.value = true
  try {
    const res = await commentApi.myComments(page, pageSize.value)
    comments.value = res.data.records
    currentPage.value = res.data.current
    total.value = res.data.total
  } catch {}
  finally { loading.value = false }
}

onMounted(() => loadData())

onMounted(() => {
  if (!import.meta.client) {
    pageEntered.value = true
    return
  }

  window.requestAnimationFrame(() => {
    pageEntered.value = true
  })
})

watch(loading, (isLoading) => {
  if (isLoading) {
    listEntered.value = false
    return
  }

  if (!import.meta.client) {
    listEntered.value = true
    return
  }

  window.requestAnimationFrame(() => {
    listEntered.value = true
  })
}, { immediate: true })
</script>

<style scoped lang="scss">
.comments-page { max-width: var(--layout-max-width); margin: 0 auto; padding: var(--layout-page-padding-y) var(--layout-page-padding-x); }
.comments-page {
  opacity: 0;
  transform: translate3d(0, 10px, 0);
  transition:
    opacity 680ms cubic-bezier(0.22, 1, 0.36, 1),
    transform 760ms cubic-bezier(0.22, 1, 0.36, 1);
}

.comments-page.page-entered {
  opacity: 1;
  transform: translate3d(0, 0, 0);
}
.page-header { margin-bottom: var(--layout-page-header-margin-bottom); }
.back-link { display: inline-flex; align-items: center; gap: .375rem; color: $color-text-muted; text-decoration: none; margin-bottom: .5rem; .dark & { color: $color-dark-text-muted; } }
.header-row { display: flex; align-items: center; justify-content: space-between; gap: .75rem; }
.header-left { display: flex; align-items: center; gap: .75rem; flex-wrap: wrap; }
.page-title {
  margin: 0;
  font-size: 1.25rem;
  font-weight: 700;
  line-height: 1.2;
  display: flex;
  align-items: center;
  gap: .35rem;
  min-height: 2rem;
  color: $color-text;
  .dark & { color: $color-dark-text; }
}
.page-total { font-size: .82rem; color: $color-text-muted; .dark & { color: $color-dark-text-muted; } }
.toolbar-btn, .manage-btn {
  display: inline-flex; align-items: center; gap: .25rem; padding: .3rem .68rem;
  border: 1px solid $color-border; border-radius: 9px; background: transparent; color: $color-text-muted; cursor: pointer;
  .dark & {
    border-color: $color-dark-border;
    background: $color-dark-bg-secondary;
    color: $color-dark-text-muted;
  }
}
.danger-btn {
  border-color: var(--status-danger-soft-border);
  color: var(--status-danger);
  background: var(--status-danger-soft-bg);
  .dark & {
    border-color: var(--status-danger-soft-border);
    color: var(--status-danger);
    background: var(--status-danger-soft-bg);
  }
}
.comment-card { position: relative; padding: .75rem .9rem; margin-bottom: .5rem; border: 1px solid $color-border; border-radius: 12px; background: $color-bg; transition: border-color .2s, box-shadow .2s; .dark & { border-color: $color-dark-border; background: $color-dark-bg-secondary; } }
.comment-card {
  opacity: 0;
  transform: translate3d(0, 12px, 0);
}

.comment-card.card-entered {
  opacity: 1;
  transform: translate3d(0, 0, 0);
  transition:
    opacity 560ms cubic-bezier(0.22, 1, 0.36, 1),
    transform 620ms cubic-bezier(0.22, 1, 0.36, 1),
    border-color .2s,
    box-shadow .2s;
  transition-delay: calc(40ms + min(var(--enter-index), 7) * 50ms);
}
.comment-card.managing { cursor: pointer; }
.comment-card.selected { border-color: $color-primary; background: rgba($color-primary,.03); }
.comment-card:hover { border-color: rgba(59,130,246,.45); box-shadow: 0 8px 18px rgba(59,130,246,.08); .dark & { border-color: rgba(148,163,184,.46); box-shadow: 0 8px 18px rgba(2, 6, 23, .35); } }
.row-top { display: flex; align-items: center; gap: .5rem; margin-bottom: .25rem; min-height: 20px; padding-right: 1.5rem; }
.post-link { font-size: .8rem; color: $color-primary; text-decoration: none; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; min-width: 0; }
.post-link.disabled { color: $color-text-muted; .dark & { color: $color-dark-text-muted; } }
.meta-likes { display: inline-flex; align-items: center; gap: .15rem; font-size: .7rem; color: $color-text-muted; .dark & { color: $color-dark-text-muted; } }
.meta-time { font-size: .7rem; color: $color-text-muted; white-space: nowrap; .dark & { color: $color-dark-text-muted; } }
.status-badge {
  padding: 0 .3rem;
  border-radius: 999px;
  font-size: .6rem;
  line-height: 1.6;
  font-weight: 500;
}
.status-badge--pending {
  background: var(--status-warning-soft-bg);
  color: var(--status-warning);
  border: 1px solid var(--status-warning-soft-border);
}
.status-badge--rejected {
  background: var(--status-danger-soft-bg);
  color: var(--status-danger);
  border: 1px solid var(--status-danger-soft-border);
}
.reject-reason {
  display: flex;
  align-items: flex-start;
  gap: .35rem;
  margin-top: .4rem;
  padding: .4rem .6rem;
  border-radius: 6px;
  font-size: .78rem;
  line-height: 1.5;
  background: var(--status-danger-soft-bg);
  color: var(--status-danger);
  border: 1px solid var(--status-danger-soft-border);
}
.comment-pending {
  border-color: rgba(245, 158, 11, .45);
}
.comment-pending:hover {
  border-color: rgba(245, 158, 11, .7);
}
.comment-rejected {
  border-color: rgba(239, 68, 68, .35);
}
.comment-rejected:hover {
  border-color: rgba(239, 68, 68, .65);
}
.comment-text { font-size: .85rem; line-height: 1.5; word-break: break-word; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; color: $color-text; .dark & { color: $color-dark-text; } }
.reply-prefix { color: #94a3b8; font-size: .8rem; .dark & { color: $color-dark-text-muted; } }
.delete-btn { position: absolute; top: .375rem; right: .375rem; width: 24px; height: 24px; border: none; border-radius: $radius-sm; background: rgba(0,0,0,.04); color: $color-text-muted; display: inline-flex; align-items: center; justify-content: center; cursor: pointer; opacity: 0; transition: opacity .2s, color .2s, background-color .2s; .dark & { background: rgba(148,163,184,.14); color: $color-dark-text-muted; } }
.comment-card:hover .delete-btn { opacity: 1; }
.delete-btn:hover,
.delete-btn:focus-visible {
  color: var(--status-danger);
  background: var(--status-danger-soft-bg);
  .dark & {
    color: var(--status-danger);
    background: var(--status-danger-soft-bg);
  }
}
.empty-state {
  text-align: center;
  padding: 4rem 1rem;
  color: #94a3b8;
  border: 1px dashed rgba(148,163,184,.45);
  border-radius: 12px;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  gap: .5rem;
  .dark & { color: $color-dark-text-muted; border-color: rgba(148,163,184,.32); background: $color-dark-bg-secondary; }
}
.fade-enter-active, .fade-leave-active { transition: opacity .2s; }
.fade-enter-from, .fade-leave-to { opacity: 0; }

@media (prefers-reduced-motion: reduce) {
  .comments-page,
  .comment-card,
  .comment-card.card-entered {
    opacity: 1;
    transform: none;
    transition: none;
  }
}
</style>
