  ...
  <div class="tag-posts-page">
    ...
    <template v-else>
      <header class="page-header">
        ...
        <h1 class="page-title">
          <span class="tag-hash" :style="{ color: tagColor }">#</span>{{ tagName }}
        </h1>
      </header>
      ...
      <section class="post-list">
        ...
        <div v-else class="empty-state">该标签下暂无文章</div>
      </section>
    </template>
  </div>
</template>
...
const tagColor = computed(() => getTagColor(tagId.value))

async function loadPosts() {
  try {
    const res = await postApi.list({ pageNum: currentPage.value, pageSize: 10, tagSlug: slug })
...
onMounted(async () => {
  try {
    const tagRes = await tagApi.getBySlug(slug)
    tagName.value = tagRes.data.name
    tagId.value = tagRes.data.id

    useHead({
      title: `#${tagName.value} - 标签 - Weblog`,
      meta: [
        { name: 'description', content: `标签 #${tagName.value} 下的所有文章` },
      ],
    })
...
.tag-posts-page { max-width: 800px; margin: 0 auto; padding: 2rem 1.5rem; }
...
.tag-hash { font-weight: 700; margin-right: 0.125rem; }
