      ...
      >
            ...
            <div v-else-if="posts.length" class="post-grid">
              <template v-for="item in postGridItems" :key="item.key">
                ...
                />
                <PromotionArticleCard v-else :ad="item.ad" class="post-grid-promo" />
              </template>
...
const fullPageSize = 20
const postBatchSize = ref(fullPageSize)
...
const backendPageSize = computed(() => {
  ...
  return Math.max(1, fullPageSize - reservedAdCount)
})
...
const postGridItems = computed<PostGridItem[]>(() => {
  ...
  const pageCount = Math.ceil(posts.value.length / postBatchSize.value)
  for (let pageNo = 1; pageNo <= pageCount; pageNo += 1) {
    const start = (pageNo - 1) * postBatchSize.value
    const end = start + postBatchSize.value
...
async function loadListCardAds() {
  ...
  listCardAdsPromise = loadListCardAdsOnce()
    .finally(() => {
...
async function loadListCardAdsOnce() {
  try {
...
async function loadPosts() {
  ...
  loading.value = true
  try {
    await loadListCardAds()
    const postApi = await getPostApi()
    postBatchSize.value = backendPageSize.value
    const res = await postApi.list({ pageNum: 1, pageSize: postBatchSize.value })
    posts.value = res.data.records
    noMore.value = res.data.records.length < postBatchSize.value || res.data.pages <= 1
    currentPage.value = 1
  } catch (error: unknown) {
...
async function loadMore() {
  ...
  try {
    ...
    const res = await postApi.list({ pageNum: nextPage, pageSize: postBatchSize.value })
    const appendedPosts = res.data.records
    posts.value = [...posts.value, ...appendedPosts]
    currentPage.value = nextPage
    noMore.value = appendedPosts.length < postBatchSize.value || nextPage >= res.data.pages
  } catch (error: unknown) {
    message.error(getErrorMessage(error, '访问受限，请稍后再试'))
  }
  finally {
