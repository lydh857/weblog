...
const sphereTags = computed<TagSphere[]>(() => {
  ...
  return tags.value.map((tag, i) => {
    ...
    const colorStr = getDisplayTagColor(tag.color, i)
...
function getDisplayTagColor(tagColor: string | null | undefined, index: number) {
  const normalizedColor = tagColor?.trim().toLowerCase()
  if (!normalizedColor || normalizedColor === '#3b82f6') {
...
function getFlatTagStyle(tagColor: string | null | undefined, index: number): Record<string, string> {
  const baseColor = getDisplayTagColor(tagColor, index)
