<template>
  <div v-if="!imageLoadFailed" class="promo-card-wrap">
    <article class="promo-card-card">
      <div v-if="applyEnabled && ad.pitEnabled" class="promo-action-float">
        <button
      ...
      >
        <div class="promo-cover-wrap" :class="{ 'is-broken': shouldShowFallback }">
          <img
            v-if="hasCoverImage"
            :src="ad.content"
            :alt="ad.title"
          ...
          >
          <div v-if="shouldShowFallback" class="promo-cover-fallback">
            <Icon name="heroicons:photo-16-solid" size="16" />
            <span>{{ fallbackText }}</span>
          </div>
          <span v-if="ad.adInfo && !shouldShowFallback" class="promo-cover-info">{{ ad.adInfo }}</span>
        </div>
        <div class="promo-content">
          <div class="promo-tags">
            <span class="promo-tag">精选推荐</span>
            <span class="promo-tag promo-tag--sponsor">广告</span>
          </div>
          <h3 class="promo-title">{{ ad.title }}</h3>
          <p class="promo-summary">{{ adSummary }}</p>
        </div>
      </component>
...
function handleClick() {
  promotionApi.recordClick(props.ad.id).catch(() => {})
}
...
function handleApplyClick() {
  ...
  if (!userStore.isLoggedIn) {
    loginModal.open('code', () => {
      promotionApplicationModal.open('post_list_card', { step: 1, pitAdId })
    })
    return
  }
  ...
  if (status === 'active' || status === 'pending' || status === 'rejected') {
    promotionApplicationModal.open('post_list_card', { step: 3, pitAdId })
    return
  }
  if (status === 'expired') {
    promotionApplicationModal.open('post_list_card', { step: 1, pitAdId })
...
async function loadMyApplicationStatus() {
  ...
  try {
    const res = await promotionApi.getMyApplication('post_list_card')
...
.promo-card-card {
  --promo-card-media-bg: linear-gradient(180deg, rgba(255, 255, 255, 0.8), rgba(248, 250, 252, 0.92));
...
.promo-cover-wrap {
  ...
  background: var(--promo-card-media-bg);
...
.promo-cover.is-error {
  opacity: 0;
}

.promo-cover-fallback {
  position: absolute;
  inset: 0;
  z-index: 3;
  display: flex;
...
.promo-cover-wrap.is-broken .promo-cover {
...
.promo-card-card:hover .promo-action-float,
.promo-card-card:focus-within .promo-action-float {
  opacity: 1;
