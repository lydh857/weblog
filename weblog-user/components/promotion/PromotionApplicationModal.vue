<template>
  <Teleport to="body">
    <Transition name="modal-fade" appear>
      <div v-if="visible" class="promotion-application-overlay" @click.self="closeModal">
        <section class="promotion-application-modal" :style="modalStyle" role="dialog" aria-modal="true" aria-label="广告投放申请">
...
import { promotionApi, type AdApplyPitOption, type AdvertisementVO } from '~/api/marketing/promotion'
...
} = usePromotionApplicationConfig()

const visible = promotionApplicationModal.visible
...
const modalStyle = computed(() => ({
  '--promotion-modal-max-width': `${modalMaxWidth.value}px`,
}))
...
function pickPreferredPositionAndPit() {
  const preferredPitId = promotionApplicationModal.preferredPitAdId.value
  if (preferredPitId) {
  ...
  const preferredPosition = normalizePosition(promotionApplicationModal.preferredPosition.value)
...
function handleImageCropped(data: { blob: Blob; url: string }) {
  ...
  pendingImageFile.value = new File([data.blob], `promotion-application.${ext}`, { type: data.blob.type })
}
...
function closeModal() {
  promotionApplicationModal.close()
}
...
async function refreshMyApplication() {
  ...
  try {
    const res = await promotionApi.getMyApplication(form.position)
...
async function handleSubmit() {
  ...
  try {
    ...
    currentStep.value = 3
    promotionApplicationModal.notifyApplicationChanged()
  } catch (error) {
...
async function initWhenOpen() {
  ...
  await loadPromotionApplicationConfig(true)
  ...
  await refreshMyApplication()
  const preferredStep = promotionApplicationModal.preferredStep.value
...
watch([
  () => promotionApplicationModal.preferredPosition.value,
  () => promotionApplicationModal.preferredPitAdId.value,
], async () => {
  if (!visible.value) return
  pickPreferredPositionAndPit()
  ensurePitSelectedForPosition()
...
.promotion-application-modal {
  ...
  width: min(var(--promotion-modal-max-width, 1120px), 100%);
  height: auto;
