            ...
            <div v-if="currentStep === 3" class="step-content">
              ...
              <template v-else-if="myLink">
                <div
                  class="notice"
                  :class="{
                    'notice-info': isPendingStatus(myLink.status),
                    'notice-success': myLink.status === 'active',
                    'notice-error': myLink.status === 'rejected',
                  }"
                >
                  <p v-if="isPendingStatus(myLink.status)">您的申请正在等待管理员审批...</p>
...
watch(() => props.visible, async (val) => {
  ...
  try {
    ...
    if (myLink.value) {
      ...
      // 待审批或已通过 → 直接跳到状态页
      if (isPendingStatus(myLink.value.status) || myLink.value.status === 'active') {
        currentStep.value = 3
      } else {
        currentStep.value = 1
      }
