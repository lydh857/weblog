...
function shouldShowLocalDebugHint() {
  if (!import.meta.client) {
    return false
  }
  if (import.meta.dev) {
...
function showModalRetryMissHintOnce() {
  if (!import.meta.client || !shouldShowLocalDebugHint()) {
    return
  }

  const key = 'announcement_modal_retry_miss_hint_v1'
  if (sessionStorage.getItem(key) === '1') {
    return
  }

  sessionStorage.setItem(key, '1')
  message.info('调试提示：信封已关闭，但弹窗公告暂无可用数据（请求与缓存均为空）')
}
