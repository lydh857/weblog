...
function createHttp(): AxiosInstance {
  ...
  instance.interceptors.response.use(
    ...
    (response: AxiosResponse) => {
      ...
      if (data.code !== 200) {
        // 仅对非认证接口的业务 401 清除 token 并弹登录框
        // 登录/注册等接口 401 是正常业务错误，不应清除登录态
        const url = response.config?.url || ''
        const isAuthEndpoint = /\/(auth\/(login|register)|user\/(bind-email|change-email|set-password|reset-password))/.test(url)
        if (data.code === 401 && import.meta.client && !isAuthEndpoint) {
          removeToken()
          try { useLoginModal().open() } catch { /* 弹窗可能因上下文不可用而失败，不影响错误流程 */ }
        }
    ...
    async (error: AxiosError<{ code?: number; message?: string }>) => {
      ...
      // 401 未授权：登录/注册等认证接口的 401 直接返回业务错误，不尝试刷新 token
      const requestUrl = originalRequest?.url || originalRequest?.baseURL || ''
      const isAuthEndpoint = /\/(auth\/(login|register)|user\/(bind-email|change-email|set-password|reset-password))/.test(requestUrl)

      if (error.response?.status === 401 && import.meta.client && originalRequest && !originalRequest._retry && !isAuthEndpoint) {
        if (isRefreshing) {
