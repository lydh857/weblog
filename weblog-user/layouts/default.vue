<template>
  <div class="layout" :class="{ dark: isDark }">
    ...
    <LazyPromotionApplicationModal v-if="shouldMountPromotionApplicationModal" />
    ...
    <Transition name="left-promotion-float-fade">
      <aside v-if="shouldShowGlobalLeftAd" class="global-left-promotion" :class="{ 'is-scrolling-hidden': isGlobalLeftAdScrollingHidden }">
        <LazyPromotionSlotPanel promotion-slot="home_left" :visible="globalLeftAdVisible" :force-rotate="true" @closed="handleGlobalLeftAdClosed" />
      </aside>
    </Transition>
...
const shouldMountPromotionApplicationModal = ref(false)
...
async function tryRememberLogin() {
  if (userStore.userInfo.userId || !hasRememberLoginEnabled()) {
    return
  }
...
watch(() => promotionApplicationModal.visible.value, (opened) => {
  if (opened) {
    shouldMountPromotionApplicationModal.value = true
  }
}, { immediate: true })
...
.global-left-promotion.is-scrolling-hidden {
  opacity: 0;
  pointer-events: none;
  transform: translate3d(-12px, -50%, 0);
}

.left-promotion-float-fade-enter-active,
.left-promotion-float-fade-leave-active {
  transition: opacity 260ms ease, transform 320ms ease;
}

.left-promotion-float-fade-enter-from,
.left-promotion-float-fade-leave-to {
  opacity: 0;
  transform: translate3d(-12px, -50%, 0);
}
...
@media (min-width: 769px) and (max-width: 1200px) {
  ...
  :global(body.with-comment-bottom-bar) .global-left-promotion {
    top: auto;
    bottom: calc(var(--comment-bottom-bar-avoidance, 0px) + env(safe-area-inset-bottom) + 16px);
    transform: none;
  }

  :global(body.with-comment-bottom-bar) .global-left-promotion.is-scrolling-hidden {
