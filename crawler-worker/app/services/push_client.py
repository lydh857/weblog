...
async def push_approved_items(session: AsyncSession, item_ids: list[int], push_mode: str = "skip") -> PushApprovedResponse:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        ingest_resp = await client.post(
            f"{target_profile['base_url']}/api/admin/crawler/v1/candidates:ingest",
            headers=_headers(),
            json={
        ...
        if candidate_ids:
            push_resp = await client.post(
                f"{target_profile['base_url']}/api/admin/crawler/v1/drafts:push",
                headers=_headers(),
        ...
        for asset in asset_rows:
            ...
            with file_path.open("rb") as fh:
                ...
                resp = await client.post(
                    f"{target_profile['base_url']}/api/admin/crawler/v1/assets:upload",
                    headers=_headers(),
                    files=files,
                )
