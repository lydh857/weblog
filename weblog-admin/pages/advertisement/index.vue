<template>
  <div class="ad-page">
    ...
    <el-tabs v-model="contentTab" class="content-tabs compact-tabs">
      ...
      <el-tab-pane label="广告顺序" name="pitOrder" />
    ...
    <div v-if="contentTab === 'pitOrder'" class="pit-order-panel">
      <div class="pit-order-head">
        <div>
          <h3>广告顺序</h3>
          <p>拖动投放中的广告即可调整用户端展示顺序；保存后会自动更新排序权重，数字越大越靠前。</p>
        </div>
        <el-button size="small" :loading="pitOrderLoading || pitOrderSaving" @click="loadPitOrderMeta">刷新</el-button>
      </div>
      ...
      <el-tabs v-model="pitOrderActiveTab" class="pit-order-tabs compact-tabs">
        ...
        >
          ...
          <div v-loading="pitOrderLoading" class="pit-order-list">
              ...
              >
                <div class="pit-order-main">
                  ...
                  <div class="pit-order-content">
                    ...
                    <span>{{ buildOrderMeta(row) }}</span>
                    <div class="pit-order-tags">
                      <el-tag size="small" effect="plain">权重 {{ row.weight ?? 0 }}</el-tag>
                      <el-tag v-if="row.pitEnabled" size="small" effect="plain" type="success">开放坑位</el-tag>
            ...
            <el-empty v-else description="当前广告位暂无投放中的广告" :image-size="64" />
          </div>
        </el-tab-pane>
      </el-tabs>
    </div>
    ...
    <template v-if="contentTab === 'list'">
      ...
      <el-table-column prop="weight" width="76" align="center">
        <template #header>
          <el-tooltip content="排序权重：用户端按权重从高到低展示；广告顺序页拖拽会自动更新。" placement="top">
            <span>排序</span>
          </el-tooltip>
        </template>
      </el-table-column>
...
<div v-if="detailRecord.advertiserId && detailRecord.status === 'pending'" class="detail-review-panel">
          ...
          <el-form label-width="82px" class="reject-form">
            <el-form-item label="拒绝原因">
              <el-select v-model="detailRejectReason" placeholder="选择预设原因" clearable style="width: 100%">
                <el-option v-for="reason in adRejectReasonOptions" :key="reason" :label="reason" :value="reason" />
              </el-select>
            </el-form-item>
            <el-form-item label="自定义">
              <el-input
                ...
                maxlength="200"
                show-word-limit
                placeholder="拒绝时必须填写原因，可选择预设后再编辑"
              />
            </el-form-item>
          </el-form>
          <div class="detail-review-actions">
            ...
            <el-button type="danger" :loading="detailSubmitting" @click="submitDetailReview('block')">拒绝并封禁</el-button>
          </div>
        </div>
      </div>
    </el-dialog>
      ...
      <div class="dialog-scroll-body">
        <el-form :model="form" label-width="92px" class="ad-form-grid">
          ...
          <el-form-item label="排序权重">
            ...
            <span class="inline-tip">数字越大越靠前；也可在“广告顺序”页拖拽调整</span>
          </el-form-item>
        </el-form>
      </div>
      <template #footer>
...
function sanitizeCodeForPreview(html: string) {
  return sanitizeHtmlWith(html, codePreviewSanitizeOptions)
}
...
function buildOrderMeta(row: AdvertisementVO) {
  const parts = [`广告ID：${row.id}`]
  if (row.adInfo) {
    parts.push(row.adInfo)
  }
  if (row.position === 'post_list_card' && row.insertAfter) {
    parts.push(`第 ${row.insertAfter} 篇后插入`)
  }
  return parts.join(' · ')
}
...
async function submitDetailReview(status: 'active' | 'rejected' | 'block') {
  if (!detailRecord.value) return

  const reason = detailRejectReason.value.trim()
  if ((status === 'rejected' || status === 'block') && !reason) {
  ...
  try {
    ...
    const nextStatus = status === 'block' ? 'rejected' : status
    await advertisementApi.updateStatus(detailRecord.value.id, nextStatus, nextStatus === 'rejected' ? reason : undefined)
    if (status === 'block' && detailRecord.value.advertiserId) {
      await logsApi.blockUser({ userId: detailRecord.value.advertiserId, durationMinutes: 1440, reason: `广告审核封禁：${reason}` })
    }
    ElMessage.success(status === 'active'
      ? (shouldAutoExtend ? '审核已通过，系统已自动顺延投放期' : '审核已通过')
      : status === 'block' ? '已拒绝并封禁24小时' : '审核已拒绝')
    await loadData()
...
async function loadData() {
  ...
  try {
    ...
    selectedIds.value = []
    selectedRows.value = []
    openFocusedAdvertisementIfNeeded()
...
async function loadPitOrderMeta() {
  ...
  try {
    ...
    for (const row of res.data.records || []) {
      if (!row || !supportedPitPositions.has(row.position)) {
        continue
      }
    ...
    for (const position of Object.keys(nextMap) as PitOrderPosition[]) {
      nextMap[position].sort((a, b) => {
        const left = Number(a.weight || 0)
        const right = Number(b.weight || 0)
...
async function handlePitDrop(position: string, targetPitId: number) {
  ...
  try {
    await advertisementApi.updateDisplayOrder({
      position,
      adIds: rows.map(item => item.id),
    })
    ElMessage.success('广告顺序已更新')
    await loadData()
  } catch (e: unknown) {
    ElMessage.error((e as Error).message || '保存广告顺序失败')
  } finally {
...
function openImageCropper() {
  if (canCropImageSource(form.content)) {
...
function openFocusedAdvertisementIfNeeded() {
  const focusId = getFocusId()
  if (!focusId || detailVisible.value) return
...
.ad-page {
  ...
  :deep(.pit-order-tabs > .el-tabs__content),
  :deep(.price-rule-tabs > .el-tabs__content) {
    flex: 0 0 auto;
  }
