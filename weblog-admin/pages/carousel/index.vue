      ...
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="90px">
        ...
        <el-form-item label="标题">
          <el-input v-model="form.title" maxlength="200" show-word-limit
            :placeholder="form.type === 'article' && articleDefaults.title ? `默认：${articleDefaults.title}` : '轮播标题（可选）'" />
          <span v-if="form.type === 'article'" class="form-tip">留空则使用文章标题</span>
          <span v-else class="form-tip">留空则只展示图片</span>
        </el-form-item>
        ...
        <el-form-item v-if="form.type === 'image'" label="跳转链接">
          <el-input v-model="form.linkUrl" placeholder="点击跳转的 URL（可选）" />
        </el-form-item>
...
/** 表单校验规则：图片类型支持纯图片轮播，只强制背景图 */
const formRules: FormRules = {
  type: [{ required: true, message: '请选择类型', trigger: 'change' }],
  imageUrl: [{
    trigger: 'change',
    validator: (_rule, value: string, callback) => {
      if (form.type === 'image' && !value) {
...
function openImageCropper() {
  if (canCropImageSource(form.imageUrl)) {
...
async function handleSubmit() {
  ...
  try {
    ...
    const data: CarouselForm = {
      ...
      articleId: form.type === 'article' ? form.articleId : null,
    }
...
async function handleDelete(row: CarouselItem) {
  await ElMessageBox.confirm(`确定删除轮播「${row.title || '未设置标题'}」？`, '提示', { type: 'warning' })
  try {
