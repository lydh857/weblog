  ...
  <el-container class="admin-layout">
    ...
    <el-aside :width="isCollapsed ? '64px' : '220px'" :class="{ 'is-collapsed': isCollapsed }" class="admin-aside">
        <div class="logo" @click="navigateTo('/')">
          <div class="logo-icon">
            <img src="/brand/logo.png" :alt="siteName + ' logo'" class="logo-icon-img app-brand-logo">
          </div>
        ...
        <el-menu-item index="/comment">
          ...
          <template #title>
            <span class="menu-title-with-badge">
              评论管理
              <span v-if="pendingCommentCount > 0" class="menu-count-badge menu-count-badge--warning">
...
import {
  ...
  SwitchButton, Fold, Expand, List, Collection, MagicStick, Timer, Lock,
...
import { commentApi } from '~/api/content/comment'
...
const siteName = computed(() => siteConfigState.value.siteName)

useHead({ titleTemplate: () => `${siteName.value} 管理端` })
...
async function loadPendingFriendLinkCount() {
  try {
    ...
    pendingFriendLinkCount.value = res.data.filter(item => item.status === 'pending' || item.status === 'pending_domain_review').length
  } catch {
...
watch(() => route.path, (path) => {
  if (path.startsWith('/user') || path.startsWith('/advertisement') || path.startsWith('/friend-link') || path.startsWith('/comment')) {
    loadMenuBadges()
  }
})
