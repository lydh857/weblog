...
type ToolbarAction = 'continue' | 'polish' | 'rewrite' | 'translate' | 'deduplicate' | 'format'
type WritingAction = ToolbarAction | 'chat'

function isToolbarAction(action: string): action is ToolbarAction {
  return action === 'continue' || action === 'polish' || action === 'rewrite' || action === 'translate' || action === 'deduplicate' || action === 'format'
}
...
const IconFormat = () => h('svg', { viewBox: '0 0 24 24', width: 14, height: 14, fill: 'none', stroke: 'currentColor', 'stroke-width': 2 }, [h('path', { d: 'M4 6h16M4 12h10M4 18h16' }), h('path', { d: 'M17 10l2 2-2 2' })])
...
const quickActions: Array<{ key: ToolbarAction; label: string; icon: () => ReturnType<typeof h>; needSelection: boolean }> = [
  ...
  { key: 'format', label: '美化格式', icon: IconFormat, needSelection: true },
...
function createWritingSse(action: WritingAction, params: Record<string, unknown>): ReturnType<typeof aiApi.writingContinue> | null {
  switch (action) {
    ...
    case 'format':
      return aiApi.writingFormat({ text: params.text as string })
    case 'chat':
