...
function appendFinalTrackPoint() {
  ...
  if (!lastPoint || Math.abs(lastPoint.x - sliderLeft.value) > 0.5) {
    trackPoints.push({ x: sliderLeft.value, y: lastPoint?.y ?? 0, timestamp: Math.max(timestamp, lastPoint?.timestamp ?? timestamp) })
  }
}
...
async function refreshCaptcha() {
  if (loading.value || verifying.value || isDragging.value) return
  clearRefreshTimer()
  loading.value = true
  status.value = 'idle'
  resultMessage.value = ''
