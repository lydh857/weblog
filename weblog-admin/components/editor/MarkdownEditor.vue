    ...
    <Teleport to="body">
      <Transition name="ft-fade">
        ...
        >
          ...
          <button title="分隔线" @mousedown.prevent="insertHorizontalRule">
            <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M4 11h16v2H4v-2Z"/></svg>
          </button>
          ...
          <button title="AI 美化格式" class="ft-ai-btn" @mousedown.prevent="doAiAction('format')">
            <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6h16M4 12h10M4 18h16"/><path d="M17 10l2 2-2 2"/></svg>
            美化格式
          </button>
        </div>
      </Transition>
...
import { applyWatermarkToFile, shouldApplyWatermark, snapshotImageWatermarkConfig, type ImageWatermarkConfig } from '~/utils/image/watermark'
...
const toolbarItems: ToolbarNames[] = [
  'bold', 'underline', 'italic', 'strikeThrough', 3,
  '-',
...
async function handleUploadImg(
  ...
  const watermarkConfig = snapshotImageWatermarkConfig(props.imageWatermark)
  const settled = await runWithConcurrency(
    files,
    IMAGE_UPLOAD_CONCURRENCY,
    async (file) => {
      const webpFile = await convertToWebp(file)
      const finalFile = watermarkConfig && shouldApplyWatermark(watermarkConfig, 'content')
        ? await applyWatermarkToFile(webpFile, watermarkConfig)
        : webpFile
