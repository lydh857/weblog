...
export interface UserVO {
  ...
  mutedPermanent: boolean
  mutedUntil: string | null
  mutedReason: string | null
  userBlocked: boolean
  userBlockPermanent: boolean
  userBlockRemainingSeconds: number
...
export const userApi = {
  ...
  mute: (userId: number, params: { permanent: boolean; minutes?: number; reason: string }) =>
    http.post(`/admin/user/${userId}/mute`, params),

  muteByEmail: (params: { email: string; permanent: boolean; minutes?: number; reason: string }) =>
    http.post('/admin/user/mute-by-email', params),

  unmute: (userId: number) =>
    http.post(`/admin/user/${userId}/unmute`),

  unmuteByEmail: (email: string) =>
    http.post('/admin/user/unmute-by-email', { email }),
