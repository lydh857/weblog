...
export interface RiskControlVO {
  id: number
  type: 'IP' | 'USER' | 'MUTE'
  targetValue: string
  userId: number | null
...
export const logsApi = {
  ...
  blockUser: (payload: { userId?: number; email?: string; durationMinutes?: number; reason?: string; permanent?: boolean }) =>
  ...
  getUserBlockStatusByEmail: (email: string) =>
    http.get<unknown, { data: UserBlockStatusVO }>('/admin/logs/user-block/by-email', { params: { email } }),
  ...
  getRiskControls: (params: {
    pageNum?: number
    pageSize?: number
    type?: 'IP' | 'USER' | 'MUTE'
    keyword?: string
    status?: 'active' | 'expired' | 'all'
  }) => http.get<unknown, { data: PageResult<RiskControlVO> }>('/admin/logs/risk-controls', { params }),
}
