...
| 组件 | 作用 | 当前项目配置 |
| --- | --- | --- |
| GitHub 仓库 | 开发主线、触发自动部署 | `https://github.com/YOUR_USERNAME/weblog.git` |
| GitHub Actions | 构建镜像、执行部署 | `.github/workflows/deploy.yml` |
| GHCR | 存放 Docker 镜像 | `ghcr.io/YOUR_USERNAME/weblog/*` |
| 自托管 Runner | 在服务器上执行部署命令 | 标签：`self-hosted, prod, your-region` |
...
| Cloudflare | DNS、CDN、SSL、WAF、R2 对象存储 | 域名示例：`example.com` |

访问路径：

| 地址 | 说明 |
| --- | --- |
| `https://example.com/` | 用户端博客首页 |
| `https://example.com/admin` | 管理后台 |
| `https://example.com/api/...` | 后端 API |
| `https://www.example.com/...` | 自动 301 到主域 `https://example.com/...` |
...
2. 一个域名，本文以当前项目配置的 `example.com` 为例。
3. 一个 GitHub 账号，并拥有仓库 `YOUR_USERNAME/weblog` 的管理权限。
...
```bash
cd /opt/weblog
git clone https://github.com/YOUR_USERNAME/weblog.git repo-tmp
...
| `example.com/admin*` | Bypass Cache 或 Respect Existing Headers |
| `example.com/_nuxt/*` | Cache Everything，Edge TTL 可设 1 月 |
| `example.com/uploads/*` | 可缓存，当前 Nginx 已设置 30 天 |
...
R2_REGION=auto
UPLOAD_BASE_URL=https://media.example.com
```
...
| Job | 镜像 |
| --- | --- |
| `build-and-push-backend` | `ghcr.io/YOUR_USERNAME/weblog/weblog-api:latest` |
| `build-and-push-user` | `ghcr.io/YOUR_USERNAME/weblog/weblog-user:latest` |
| `build-and-push-admin` | `ghcr.io/YOUR_USERNAME/weblog/weblog-admin:latest` |

部署 Job：

```yaml
deploy-to-server:
  runs-on: [self-hosted, prod, your-region]
```
...
./config.sh --url https://github.com/YOUR_USERNAME/weblog --token <GitHub 页面给你的 Runner Token> --labels self-hosted,prod,your-region
```

注意 labels 必须包含：

```text
self-hosted,prod,your-region
```
...
```bash
echo "<GitHub PAT>" | docker login ghcr.io -u YOUR_USERNAME --password-stdin
```
...
| Secret | 当前项目示例 | 说明 |
| --- | --- | --- |
| `ENV_PROD_CORS_ORIGINS` | `https://example.com,https://www.example.com` | CORS 允许来源 |
| `ENV_PROD_ALLOWED_ORIGINS` | `https://example.com,https://www.example.com` | 安全校验允许来源 |
| `ENV_PROD_API_BASE_URL` | `https://example.com` | 前端浏览器访问 API 的站点根地址，不带 `/api` |
| `ENV_PROD_UPLOAD_BASE_URL` | `https://example.com/uploads` | 本地上传访问地址 |
| `ENV_PROD_OUTBOUND_LINK_ALLOWED_DOMAINS` | `example.com,www.example.com,github.com,gitee.com` | 允许的外链域名白名单 |

注意：`ENV_PROD_API_BASE_URL` 只写协议和域名，不要写成 `https://example.com/api`。Compose 会自动拼接 `/api`：
...
| `ENV_PROD_UPLOAD_BASE_URL` | `https://example.com/uploads` |
...
| `ENV_PROD_R2_PUBLIC_BASE_URL` | `https://media.example.com` |
| `ENV_PROD_R2_REGION` | `auto` |
| `ENV_PROD_UPLOAD_BASE_URL` | `https://media.example.com` |
...
| Authorization callback URL | `https://example.com/api/oauth/github/callback` |
...
```bash
curl -I https://example.com/api/portal/post/recent?pageNum=1\&pageSize=1
```
...
```bash
docker pull ghcr.io/YOUR_USERNAME/weblog/weblog-api:latest
docker pull ghcr.io/YOUR_USERNAME/weblog/weblog-user:latest
docker pull ghcr.io/YOUR_USERNAME/weblog/weblog-admin:latest
```
...
docker create --name admin-temp ghcr.io/YOUR_USERNAME/weblog/weblog-admin:latest
...
```text
ghcr.io/YOUR_USERNAME/weblog/weblog-api:<commit-sha>
```
...
不要把 `API_BASE_URL` 写成 `https://example.com/api`。
...
| Runner 标签正确 | `self-hosted, prod, your-region` |
...
| 管理端 | `https://example.com/admin` |
| API | `https://example.com/api/portal/post/recent?pageNum=1&pageSize=1` |
...
| 项 | 当前值 |
| --- | --- |
| GitHub 主仓库 | `https://github.com/YOUR_USERNAME/weblog.git` |
...
| 管理端路径 | `/admin` |
| API 路径 | `/api` |
| 后端镜像 | `ghcr.io/YOUR_USERNAME/weblog/weblog-api:latest` |
| 用户端镜像 | `ghcr.io/YOUR_USERNAME/weblog/weblog-user:latest` |
| 管理端镜像 | `ghcr.io/YOUR_USERNAME/weblog/weblog-admin:latest` |
| 后端容器 | `weblog-api` |
| 用户端容器 | `weblog-user` |
...
| MySQL 端口 | `3306`，仅本机绑定 |
| Redis 端口 | `6379`，仅本机绑定 |
| Runner 标签 | `self-hosted, prod, your-region` |
...
1. 买服务器，安装 Docker。
2. 把域名 `example.com` 接入 Cloudflare。
...
8. 在服务器安装 GitHub self-hosted runner，标签设置为 `self-hosted,prod,your-region`。
9. 推送代码到 GitHub `master`。
10. 打开 GitHub Actions 看部署是否成功。
11. 浏览器访问 `https://example.com/` 和 `https://example.com/admin`。
