...
public class ExternalLinkGovernanceService {
    ...
    public LinkCheckResult trustDomainForReviewedLink(String linkUrl, String reviewer, String reviewReason) {
        LinkCheckResult initialCheck = evaluateForSubmission(linkUrl, true);
        if (initialCheck.trusted()) {
            return initialCheck;
        }
        String domain = initialCheck.domain();
        LinkDomainPolicy policy = domain == null ? null : findBestPolicy(domain);
        if (policy == null) {
            throw new BusinessException(ResultCode.BAD_REQUEST, "外链域名策略不存在，不能通过友链申请");
        }
        policy.setStatus("trusted");
        ...
        policy.setExpireAt(null);
        policy.setSource("manual");
        linkDomainPolicyMapper.updateById(policy);
        log.info("外链域名快捷信任: domain={}, reviewer={}", policy.getDomain(), policy.getReviewer());
        return evaluateForSubmission(linkUrl, false);
    }
