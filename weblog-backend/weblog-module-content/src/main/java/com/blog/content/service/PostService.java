...
public class PostService {
    ...
    public PostVO create(PostCreateRequest req, Long authorId) {
        ...
        post.setTopicOnly(Boolean.TRUE.equals(req.getTopicOnly()));
        post.setViewCount(0);
        post.setLikeCount(0);
    ...
    public int batchSetTop(List<Long> ids, boolean isTop, Long operatorId) {
        ...
        int count = postMapper.batchSetTop(ids, isTop, operatorId);
    ...
    public int batchSetDisabled(List<Long> ids, boolean isDisabled, Long operatorId) {
        if (ids == null || ids.isEmpty()) return 0;
        int count = postMapper.batchSetDisabled(ids, isDisabled, operatorId);
    ...
    public int batchPublish(List<Long> ids, Long operatorId, boolean isAdmin) {
        ...
        // 单条批量 SQL 更新状态
        int count = isAdmin
                ? postMapper.batchPublishByAdmin(ids)
                : postMapper.batchPublishByAuthor(ids, operatorId);

        if (count == 0) {
            log.info("批量发布: count=0, operator={}, isAdmin={}", operatorId, isAdmin);
            return 0;
        }

        // 批量查询已发布的文章用于索引
        List<Post> publishedPosts = postMapper.selectList(
                new LambdaQueryWrapper<Post>()
                        .in(Post::getId, ids)
                        .eq(Post::getStatus, "published")
                        .eq(Post::getIsPublished, true));

        if (!publishedPosts.isEmpty()) {
            Set<Long> publishedIds = publishedPosts.stream().map(Post::getId).collect(Collectors.toSet());
            List<PostContent> contents = postContentMapper.selectList(
                    new LambdaQueryWrapper<PostContent>().in(PostContent::getId, publishedIds));
            Map<Long, String> contentMap = contents.stream()
                    .collect(Collectors.toMap(PostContent::getId, pc -> pc.getContent() != null ? pc.getContent() : "", (a, b) -> a));

            for (Post post : publishedPosts) {
                String content = contentMap.getOrDefault(post.getId(), "");
                if (shouldIndexPost(post)) {
                    postIndexService.indexPost(post.getId(), post.getSlug(), post.getTitle(), content,
                            post.getSummary(), post.getCategoryId(), post.getAuthorId());
                } else {
                    postIndexService.deleteIndex(post.getId());
                }
            }
        }
    ...
    public List<PostVO> listTodayPosts(int limit) {
        ...
        LambdaQueryWrapper<Post> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(Post::getStatus, "published")
               .and(w -> w.eq(Post::getTopicOnly, false).or().isNull(Post::getTopicOnly))
    ...
    private void excludeTopicOnly(LambdaQueryWrapper<Post> wrapper) {
        wrapper.and(w -> w.eq(Post::getTopicOnly, false).or().isNull(Post::getTopicOnly));
    }

    private boolean shouldIndexPost(Post post) {
        return "published".equals(post.getStatus()) && !Boolean.TRUE.equals(post.getTopicOnly());
    }
    ...
    private void handleNewCategory(PostCreateRequest req, Post post) {
        ...
        if ("draft".equals(req.getStatus())) return;
        ...
        if (parentId == 0L) {
            ...
            if (req.getNewChildCategoryName() != null && !req.getNewChildCategoryName().isBlank()) {
                Category child = categoryService.create(req.getNewChildCategoryName().trim(), null, null, newCat.getId(), null);
                post.setSubCategoryId(child.getId());
            }
        } else {
        ...
        while (matcher.find()) {
            ...
            if (isTemp) {
                try {
                    ...
                    updateMovedMediaResource(objectKey, formalUrl);
    ...
    private void updateMovedMediaResource(String oldObjectKey, String formalUrl) {
        if (ossResourceService == null || oldObjectKey == null || formalUrl == null) {
            return;
        }
        String newObjectKey = storageFacade.extractObjectKey(formalUrl);
        if (newObjectKey == null || newObjectKey.isBlank()) {
            return;
        }
        ossResourceService.updateMovedResource(oldObjectKey, newObjectKey, formalUrl);
    }
    ...
    private PostVO toVO(Post post, boolean withContent, Map<Long, Category> categoryMap, Map<Long, List<Tag>> postTagsMap) {
        ...
        vo.setTopicOnly(post.getTopicOnly());
    ...
    public int batchRestore(List<Long> ids) {
        if (ids == null || ids.isEmpty()) return 0;
        int count = postMapper.batchRestore(ids);
    ...
    public int batchPermanentDelete(List<Long> ids) {
        if (ids == null || ids.isEmpty()) return 0;
        int count = postMapper.batchPermanentDelete(ids);
        if (count > 0) {
            // 批量清理内容与标签关联
            postContentMapper.delete(new LambdaQueryWrapper<PostContent>().in(PostContent::getId, ids));
            postTagMapper.delete(new LambdaQueryWrapper<PostTag>().in(PostTag::getPostId, ids));
        }
