...
public class AdvertisementService {
    ...
    public void updateDisplayOrder(String position, List<Long> orderedIds) {
        String normalized = normalizePosition(position);
        if (normalized == null || normalized.isBlank()) {
            throw new BusinessException(ResultCode.BAD_REQUEST, "广告位置不能为空");
        }
        List<Long> normalizedIds = normalizePositiveUniqueIds(orderedIds);
        if (orderedIds == null || normalizedIds.isEmpty() || normalizedIds.size() != orderedIds.size()) {
            throw new BusinessException(ResultCode.BAD_REQUEST, "广告顺序包含重复或非法ID");
        }

        List<Advertisement> currentRecords = advertisementMapper.selectList(
                new LambdaQueryWrapper<Advertisement>()
                        .eq(Advertisement::getPosition, normalized)
                        .eq(Advertisement::getStatus, "active"));
        if (currentRecords.isEmpty()) {
            throw new BusinessException(ResultCode.BAD_REQUEST, "当前广告位没有投放中的广告");
        }

        Set<Long> currentIds = new LinkedHashSet<>();
        for (Advertisement record : currentRecords) {
            if (record != null && record.getId() != null) {
                currentIds.add(record.getId());
            }
        }
        if (currentIds.size() != normalizedIds.size() || !currentIds.containsAll(normalizedIds)) {
            throw new BusinessException(ResultCode.BAD_REQUEST, "提交的广告集合与当前投放中广告不一致，请刷新后重试");
        }

        int nextWeight = normalizedIds.size();
        for (Long id : normalizedIds) {
            Advertisement update = new Advertisement();
            update.setId(id);
            update.setWeight(nextWeight--);
    ...
    private List<Long> normalizePositiveUniqueIds(List<Long> ids) {
        if (ids == null || ids.isEmpty()) {
            return List.of();
        }
    ...
    public int batchRestore(List<Long> ids) {
        ...
        return advertisementMapper.batchRestore(ids);
    }
    ...
    public int batchPermanentDelete(List<Long> ids) {
        if (ids == null || ids.isEmpty()) return 0;
        return advertisementMapper.batchPermanentDelete(ids);
    }
