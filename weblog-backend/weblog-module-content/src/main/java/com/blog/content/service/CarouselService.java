...
public class CarouselService {
  ...
  public Carousel create(CarouselCreateRequest request) {
    ...
    carousel.setTitle(normalizeOptionalText(request.getTitle()));
    carousel.setDescription(normalizeOptionalText(request.getDescription()));
    carousel.setImageUrl(request.getImageUrl());
    carousel.setLinkUrl(safeLinkUrl);
    carousel.setArticleId("article".equals(request.getType()) ? request.getArticleId() : null);
  ...
  public void update(Long id, CarouselUpdateRequest request) {
    ...
    String effectiveLinkUrl = request.getLinkUrl() != null
      ...
      : existing.getLinkUrl();
    String effectiveImageUrl = request.getImageUrl() != null ? request.getImageUrl() : existing.getImageUrl();

    if ("image".equals(effectiveType) && (effectiveImageUrl == null || effectiveImageUrl.isBlank())) {
      throw new BusinessException(ResultCode.BAD_REQUEST, "图片类型必须上传背景图");
    }
    // article 类型校验 articleId 有效性
    ...
    if (request.getTitle() != null) existing.setTitle(normalizeOptionalText(request.getTitle()));
    if (request.getDescription() != null) existing.setDescription(normalizeOptionalText(request.getDescription()));
    ...
    if ("image".equals(effectiveType)) existing.setArticleId(null);
    else if (request.getArticleId() != null) existing.setArticleId(request.getArticleId());
  ...
  private CarouselVO toVO(Carousel carousel, Map<Long, Post> postMap) {
    ...
    vo.setIsEnabled(carousel.getIsEnabled());
  ...
  private String normalizeOptionalText(String text) {
    ...
    return XssUtil.cleanText(text.trim());
  }
