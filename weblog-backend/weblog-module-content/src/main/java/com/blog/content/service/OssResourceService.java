...
public class OssResourceService {
    ...
    public void updateMovedResource(String oldFilePath, String newFilePath, String newUrl) {
        if (oldFilePath == null || oldFilePath.isBlank()
                || newFilePath == null || newFilePath.isBlank()
                || newUrl == null || newUrl.isBlank()) {
            return;
        }
        String fileType = extractFileType(newFilePath);
        ossResourceMapper.update(null, new LambdaUpdateWrapper<OssResource>()
                .eq(OssResource::getFilePath, oldFilePath)
                .set(OssResource::getFilePath, newFilePath)
                .set(OssResource::getUrl, newUrl)
                .set(!fileType.isBlank(), OssResource::getFileType, fileType));
    }
    ...
    /**
     * 查询资源列表，用于需要按归一化引用 key 做内存筛选的场景。
     */
    public List<OssResource> list(Long uploaderId, String usageType) {
        LambdaQueryWrapper<OssResource> wrapper = new LambdaQueryWrapper<>();
        if (uploaderId != null) {
            wrapper.eq(OssResource::getUploaderId, uploaderId);
        }
        ...
        wrapper.orderByDesc(OssResource::getCreateTime);
        return ossResourceMapper.selectList(wrapper);
    }
