...
public class TopicService {
    ...
    public int batchRestore(List<Long> ids) {
        ...
        int count = topicMapper.batchRestore(ids);
        if (count > 0) {
            topicCatalogMapper.batchRestoreByTopicIds(ids);
        }
    ...
    public int batchPermanentDelete(List<Long> ids) {
        if (ids == null || ids.isEmpty()) return 0;
        int count = topicMapper.batchPermanentDelete(ids);
        if (count > 0) {
            topicCatalogMapper.batchPermanentDeleteByTopicIds(ids);
        }
