...
public interface TopicMapper extends BaseMapper<Topic> {
    ...
    /** 分页查询已软删除的专题（回收站） */
    @Select("<script>" +
            "SELECT id, title, cover, summary, weight, is_publish, is_top, is_deleted, create_time, update_time " +
            "FROM t_topic WHERE is_deleted = 1" +
    ...
    /** 批量恢复软删除的专题 */
    @Update("<script>" +
            "UPDATE t_topic SET is_deleted = 0, is_publish = 0 WHERE is_deleted = 1 AND id IN " +
    ...
    /** 批量物理删除专题 */
    @Delete("<script>" +
            "DELETE FROM t_topic WHERE is_deleted = 1 AND id IN " +
            "<foreach collection='ids' item='id' open='(' separator=',' close=')'>#{id}</foreach>" +
            "</script>")
