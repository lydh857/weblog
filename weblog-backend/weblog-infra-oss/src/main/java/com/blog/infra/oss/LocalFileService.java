...
public class LocalFileService {
    ...
    public String upload(InputStream inputStream, String originalFilename, long fileSize) {
        ValidatedUpload upload = validateAndWrap(inputStream, originalFilename, fileSize);
        String objectKey = buildObjectKey("file." + upload.ext());
        saveFile(upload.inputStream(), objectKey);
        return baseUrl + "/" + objectKey;
    }

    public String uploadTemp(InputStream inputStream, String originalFilename, long fileSize) {
        ValidatedUpload upload = validateAndWrap(inputStream, originalFilename, fileSize);
        String objectKey = buildTempObjectKey("file." + upload.ext());
        saveFile(upload.inputStream(), objectKey);
        return baseUrl + "/" + objectKey;
    }
    ...
    private ValidatedUpload validateAndWrap(InputStream inputStream, String filename, long fileSize) {
        ...
        try {
            ...
            return new ValidatedUpload(bis, detectedExt);
        } catch (IOException e) {
    ...
    private record ValidatedUpload(InputStream inputStream, String ext) {}
}
