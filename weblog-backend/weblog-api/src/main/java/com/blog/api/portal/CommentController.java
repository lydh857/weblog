...
private final AdminCommentService adminCommentService;
    ...
    public Result<CommentVO> create(@Valid @RequestBody CreateCommentRequest req) {
        ...
        if (auditEnabled) {
            adminCommentService.sendPendingReviewEmail(comment.getId());
        }
...
@Operation(summary = "删除评论（只能删自己的）")
    ...
    private void assertUserCanComment(User user) {
        if (user == null) {
            throw new BusinessException(ResultCode.USER_NOT_FOUND, "用户不存在");
        }
        if (Boolean.TRUE.equals(user.getMutedPermanent())) {
            throw new BusinessException(ResultCode.FORBIDDEN, "账号已被永久禁言，无法发表评论");
        }
        LocalDateTime mutedUntil = user.getMutedUntil();
        if (mutedUntil != null && mutedUntil.isAfter(LocalDateTime.now())) {
            throw new BusinessException(ResultCode.FORBIDDEN, "账号已被禁言至" + mutedUntil.toString().replace('T', ' '));
        }
    }
