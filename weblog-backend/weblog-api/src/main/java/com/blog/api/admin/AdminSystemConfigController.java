...
public class AdminSystemConfigController {
    ...
    private static final int MAX_ALLOWED_EMAIL_DOMAINS_LENGTH = 1000;
    ...
    public Result<Void> batchUpdate(@RequestBody Map<String, String> configs, HttpServletRequest request) {
        ...
        validateAllowedEmailDomains(configs);
    ...
    private String buildAuditSummary(Map<String, String> configs) {
        ...
        trackedKeys.add(EmailDomainPolicyService.ALLOWED_EMAIL_DOMAINS_KEY);
        ...
        String summary = "系统安全配置变更: " + String.join("; ", changeItems);
    ...
    private void validateAllowedEmailDomains(Map<String, String> configs) {
        if (configs == null || !configs.containsKey(EmailDomainPolicyService.ALLOWED_EMAIL_DOMAINS_KEY)) {
            return;
        }
        String raw = configs.get(EmailDomainPolicyService.ALLOWED_EMAIL_DOMAINS_KEY);
        if (raw == null || raw.isBlank()) {
            throw new BusinessException(ResultCode.PARAM_INVALID, "允许邮箱后缀白名单不能为空");
        }
        if (raw.length() > MAX_ALLOWED_EMAIL_DOMAINS_LENGTH) {
            throw new BusinessException(ResultCode.PARAM_INVALID, "允许邮箱后缀白名单长度不能超过" + MAX_ALLOWED_EMAIL_DOMAINS_LENGTH + "个字符");
        }

        List<String> domains = java.util.Arrays.stream(raw.split(","))
        ...
        List<String> invalidDomains = domains.stream()
                .filter(item -> !EMAIL_DOMAIN_PATTERN.matcher(item).matches())
                .toList();
        if (!invalidDomains.isEmpty()) {
            throw new BusinessException(ResultCode.PARAM_INVALID, "存在非法邮箱后缀: " + String.join(",", invalidDomains));
        }
        configs.put(EmailDomainPolicyService.ALLOWED_EMAIL_DOMAINS_KEY, String.join(",", domains));
    }
