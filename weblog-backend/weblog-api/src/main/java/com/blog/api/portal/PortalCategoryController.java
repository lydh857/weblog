...
public class PortalCategoryController {
    ...
    public Result<List<CategoryTreeVO>> tree() {
        ...
        Map<Long, Long> categoryCount = buildCategoryPostCount(posts);
    ...
    static Map<Long, Long> buildCategoryPostCount(List<Post> posts) {
        Map<Long, Long> categoryCount = posts.stream()
                .filter(p -> p != null && p.getCategoryId() != null && p.getSubCategoryId() == null)
    ...
    List<CategoryTreeVO> buildTree(Long parentId, Map<Long, List<Category>> grouped,
