...
import com.blog.common.util.AesUtil;
...
public class AiConfigService {
  ...
  public void loadFromDb() {
    try {
      ...
      if (config.getApiKey() != null && !config.getApiKey().isBlank()) {
        String dbKey = config.getApiKey();
        if (AesUtil.isEncrypted(dbKey)) {
          try {
            config.setApiKey(AesUtil.decrypt(dbKey, encryptKey));
          } catch (Exception e) {
            log.error("解密数据库 AI API Key 失败，将作为明文尝试", e);
          }
        }
      }
  ...
  public void updateConfig(AiConfig update) {
    ...
    if (update.getApiKey() != null && !update.getApiKey().isBlank()) {
      update.setApiKey(AesUtil.encrypt(update.getApiKey(), encryptKey));
    }
  ...
  private void applyToProperties(AiConfig config) {
    ...
    // 如果有有效的 API Key 和 Base URL，覆盖内存配置以生效，同时避免空值覆盖
    if (config.getApiKey() != null && !config.getApiKey().isBlank() && !"disabled-placeholder".equals(config.getApiKey())) {
      aiProperties.setApiKey(config.getApiKey());
    }
    if (config.getBaseUrl() != null && !config.getBaseUrl().isBlank()) {
      aiProperties.setBaseUrl(config.getBaseUrl());
    }
