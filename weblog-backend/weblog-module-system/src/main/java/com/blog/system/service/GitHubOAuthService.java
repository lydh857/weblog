...
public class GitHubOAuthService {
    ...
    public LoginResponse handleCallback(String code, String state, String clientIp) {
        ...
        String email = getGitHubUserPrimaryEmail(accessToken);
        log.info("GitHub /user/emails 获取已验证邮箱: githubId={}, email={}", githubId, DesensitizeUtil.email(email));
        if (email == null || email.isBlank()) {
            throw new BusinessException(ResultCode.BAD_REQUEST, "无法获取 GitHub 账号邮箱，请公开或验证邮箱后重试，或使用邮箱注册后再绑定第三方账号");
        }
        email = email.trim().toLowerCase(java.util.Locale.ROOT);
        emailDomainPolicyService.assertAllowed(email);
